        ...
        <th>분류</th>
        	<td>
        		<c:if test="${sMid != 'admin'}">
        			<input type="text" name="part" id="part" value="${vo.part}" readonly autofocus required class="form-control" />
        		</c:if>
						<c:if test="${sMid == 'admin'}">
	        		<select name="part" id="part" class="form-control">
	            	<option ${vo.part == '공지사항'  ? 'selected' : ''}>공지사항</option>
	            	<option ${vo.part == 'FAQ'  ? 'selected' : ''}>FAQ</option>
	            	<option ${vo.part == '이벤트'  ? 'selected' : ''}>이벤트</option>
   		        </select>
            </c:if>
          </td>
      </tr>
