...
public class BoardController {
	...
	public String boardDeleteGet(int idx,
		...
		else return "redirect:/message/boardDeleteNo?idx="+idx+"&pag="+pag+"&pageSize="+pageSize+"&part="+part;
	}
	...
	public String BoardReplyDeletePost(int idx) {
		int res = boardService.setBoardReplyDelete(idx);
		if(res != 0) return "redirect:/message/boardReplyDeleteOk";
		else return "redirect:/message/boardReplyDeleteNo";
	}
	...
	public String boardSearchGet(Model model, String search, String part,
			@RequestParam(name="searchString", defaultValue = "", required = false) String searchString,
			@RequestParam(name="pag", defaultValue = "1", required = false) int pag,
			@RequestParam(name="pageSize", defaultValue = "10", required = false) int pageSize) {
		...
		List<BoardVO> vos = boardService.getBoardSearchList(pageVO.getStartIndexNo(), pageSize, part, search, searchString_);

		String searchTitle = "";
		if(pageVO.getSearch().equals("title")) searchTitle = "글제목";
		else if(pageVO.getSearch().equals("mid")) searchTitle = "글쓴이";
		...
		model.addAttribute("search", search);
		model.addAttribute("searchString", searchString_);
